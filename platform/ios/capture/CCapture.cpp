...
C_IMPL void captureExposureAtPoint(CCapture *capture, float x, float y)
{
    c_to_cpp(capture).exposureAtPoint(x, y);
}

C_IMPL void captureSetFlashMode(CCapture *capture, int32_t mode)
{
    c_to_cpp(capture).setFlashMode(mode);
}

C_IMPL int captureGetFlashMode(CCapture *capture)
{
    return c_to_cpp(capture).getFlashMode();
}

C_IMPL bool isAdjustingFocus(CCapture *capture)
{
    return c_to_cpp(capture).isAdjustingFocus();
}

C_IMPL bool isAdjustingExposure(CCapture *capture)
{
    return c_to_cpp(capture).isAdjustingExposure();
}
...
C_IMPL int captureSaveToCameraRoll( const char *fileName, const uint8_t* data, int size, int orientation)
{
